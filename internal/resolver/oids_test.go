...
func TestGetOidParentSymbol(t *testing.T) {
	...
	t.Run("OidComponentName local def", func(t *testing.T) {
		...
		localCtx := newResolverContext(nil, model.ResolverNormal, model.DefaultConfig())
	...
	t.Run("Normal mode resolves SMI global root", func(t *testing.T) {
		...
		normalCtx := newResolverContext(nil, model.ResolverNormal, model.DefaultConfig())
	...
	t.Run("Strict mode rejects unimported SMI global", func(t *testing.T) {
		vendorMod := &module.Module{Name: "VENDOR-MIB"}
		strictCtx := newResolverContext(nil, model.ResolverStrict, model.VerboseConfig())
...
func TestCheckSmiv2IdentifierHyphens(t *testing.T) {
	t.Run("SMIv2 with hyphen emits diagnostic", func(t *testing.T) {
		...
		// Use verbose reporting so model.SeverityWarning diagnostics are reported.
		...
		testutil.Equal(t, model.SeverityWarning, d.Severity, "severity")
...
func TestResolveNumericComponent(t *testing.T) {
	t.Run("nil parent creates root-level node", func(t *testing.T) {
		...
		testutil.Equal(t, model.GetOrCreateChild(ctx.mib.Root(), 1), node, "expected same node as Builder.GetOrCreateRoot(1)")
	})

	t.Run("creates child of existing parent", func(t *testing.T) {
		ctx := newTestContext()
		parent := model.GetOrCreateChild(ctx.mib.Root(), 1) // iso
...
func TestShouldPreferModule(t *testing.T) {
	t.Run("nil currentMod always prefers new", func(t *testing.T) {
		...
		ctx := newTestContext()
		ctx.moduleToResolved = map[*module.Module]*model.Module{srcMod: newMod}
	...
	t.Run("nil currentSrcMod prefers new", func(t *testing.T) {
		srcMod := &module.Module{Name: "NEW-MIB", Language: types.LanguageSMIv1}
		newMod := model.NewModule("NEW-MIB")
		currentMod := model.NewModule("OLD-MIB")
		ctx := newTestContext()
		ctx.moduleToResolved = map[*module.Module]*model.Module{srcMod: newMod}
		ctx.resolvedToModule = map[*model.Module]*module.Module{} // currentMod not mapped
	...
	t.Run("SMIv2 preferred over SMIv1", func(t *testing.T) {
		...
		ctx.resolvedToModule = map[*model.Module]*module.Module{oldMod: oldSrc, newMod: newSrc}
	...
	t.Run("base module beats vendor module", func(t *testing.T) {
		...
		baseMod := model.NewModule("SNMPv2-SMI")
		vendorMod := model.NewModule("VENDOR-MIB")

		ctx := newTestContext()
		ctx.moduleToResolved = map[*module.Module]*model.Module{baseSrc: baseMod, vendorSrc: vendorMod}
		ctx.resolvedToModule = map[*model.Module]*module.Module{vendorMod: vendorSrc, baseMod: baseSrc}
	...
	t.Run("base module beats vendor even with newer LAST-UPDATED", func(t *testing.T) {
		...
		baseMod := model.NewModule("RFC1155-SMI")
		vendorMod := model.NewModule("VENDOR-MIB")
	...
	t.Run("equal rank and timestamp uses lexicographic module name", func(t *testing.T) {
		...
		aMod := model.NewModule("ALPHA-MIB")
		bMod := model.NewModule("BRAVO-MIB")

		ctx := newTestContext()
		ctx.moduleToResolved = map[*module.Module]*model.Module{aSrc: aMod, bSrc: bMod}
		ctx.resolvedToModule = map[*model.Module]*module.Module{aMod: aSrc, bMod: bSrc}
...
func TestFinalizeOidDefinition(t *testing.T) {
	tests := []struct {
		...
		{"ObjectType", defObjectType, model.KindScalar},
		{"ModuleIdentity", defModuleIdentity, model.KindNode},
		{"ObjectIdentity", defObjectIdentity, model.KindNode},
		{"ValueAssignment", defValueAssignment, model.KindNode},
		{"Notification", defNotification, model.KindNotification},
		{"ObjectGroup", defObjectGroup, model.KindGroup},
		{"NotificationGroup", defNotificationGroup, model.KindGroup},
		{"ModuleCompliance", defModuleCompliance, model.KindCompliance},
		{"AgentCapabilities", defAgentCapabilities, model.KindCapability},
	}
...
func TestFinalizeOidDefinitionNameModuleSync(t *testing.T) {
	...
	t.Run("non-preferred module does not overwrite name", func(t *testing.T) {
		...
		ctx := newTestContextForModules(model.DefaultConfig(), preferredSrc, loserSrc)
	...
	t.Run("non-preferred module does not overwrite kind", func(t *testing.T) {
		// SMIv2 module defines as OBJECT-IDENTITY (model.KindNode) first
		preferredSrc := &module.Module{Name: "PREFERRED-MIB", Language: types.LanguageSMIv2}
		// SMIv1 module defines as OBJECT-TYPE (model.KindScalar) second
		loserSrc := &module.Module{Name: "LOSER-MIB", Language: types.LanguageSMIv1}
		ctx := newTestContextForModules(model.DefaultConfig(), preferredSrc, loserSrc)
		preferredMod := ctx.moduleToResolved[preferredSrc]
		...
		testutil.Equal(t, model.KindNode, node.Kind(), "kind after preferred")
		...
		testutil.Equal(t, model.KindNode, node.Kind(), "kind should not be overwritten by non-preferred module")
	...
	t.Run("preferred module does overwrite name", func(t *testing.T) {
		...
		ctx := newTestContextForModules(model.DefaultConfig(), olderSrc, newerSrc)
...
func TestResolveOids(t *testing.T) {
	t.Run("resolves qualified dependencies across modules", func(t *testing.T) {
		...
		ctx := newTestContextForModules(model.VerboseConfig(), parentMod, childMod)

		resolveOids(ctx)

		parentNode, ok := ctx.lookupNode(parentMod, "parentRoot")
		testutil.True(t, ok, "parentRoot should resolve")
		testutil.True(t, parentNode.OID().Equal(model.OID{1, 3, 6, 1, 4, 1, 55555}), "parentRoot OID")

		childNode, ok := ctx.lookupNode(childMod, "childNode")
		testutil.True(t, ok, "childNode should resolve")
		testutil.True(t, childNode.OID().Equal(model.OID{1, 3, 6, 1, 4, 1, 55555, 7}), "childNode OID")
	...
	t.Run("resolves trap types after OID definitions", func(t *testing.T) {
		...
		testutil.True(t, enterpriseNode.OID().Equal(model.OID{1, 3, 6, 1, 4, 1, 424242}), "vendorEnterprise OID")

		trapNode, ok := ctx.lookupNode(mod, "vendorTrap")
		testutil.True(t, ok, "vendorTrap should resolve")
		testutil.True(t, trapNode.OID().Equal(model.OID{1, 3, 6, 1, 4, 1, 424242, 0, 11}), "vendorTrap OID")
		testutil.Equal(t, model.KindNotification, trapNode.Kind(), "vendorTrap kind")
		noDiag(t, ctx.Diagnostics(), types.DiagOidOrphan, types.DiagOidRecursive)
	})
...
func TestFinalizeModuleIdentityOIDOnlySetForPreferred(t *testing.T) {
	...
	ctx := newTestContextForModules(model.DefaultConfig(), v2Src, v1Src)
...
func TestResolveTrapTypeDefinitions_GenericTraps(t *testing.T) {
	...
	genericTraps := []struct {
		...
		{"coldStart", 0, model.OID{1, 3, 6, 1, 6, 3, 1, 1, 5, 1}},
		{"warmStart", 1, model.OID{1, 3, 6, 1, 6, 3, 1, 1, 5, 2}},
		{"linkDown", 2, model.OID{1, 3, 6, 1, 6, 3, 1, 1, 5, 3}},
		{"linkUp", 3, model.OID{1, 3, 6, 1, 6, 3, 1, 1, 5, 4}},
		{"authenticationFailure", 4, model.OID{1, 3, 6, 1, 6, 3, 1, 1, 5, 5}},
		{"egpNeighborLoss", 5, model.OID{1, 3, 6, 1, 6, 3, 1, 1, 5, 6}},
	}
...
func TestResolveTrapTypeDefinitions_EnterpriseSpecific(t *testing.T) {
	...
	model.SetNodeName(enterpriseNode, "cisco")
	...
	wantOID := model.OID{1, 3, 6, 1, 4, 1, 9, 0, 42}
	got := node.OID()
	testutil.True(t, got.Equal(wantOID), "vendorTrap: OID")
	testutil.Equal(t, model.KindNotification, node.Kind(), "vendorTrap: kind")
}
...
func TestResolveTrapTypeDefinitions_PreferenceGating(t *testing.T) {
	...
	trapNode := model.GetOrCreateChild(model.GetOrCreateChild(enterpriseNode, 0), 1)
	testutil.Equal(t, "preferredName", trapNode.Name(), "name should reflect preferred module")
	testutil.Equal(t, model.KindNotification, trapNode.Kind(), "kind")
...
func TestIsSnmpTrapsOID(t *testing.T) {
	tests := []struct {
		...
		{"snmpTraps", model.OID{1, 3, 6, 1, 6, 3, 1, 1, 5}, true},
		{"enterprises", model.OID{1, 3, 6, 1, 4, 1}, false},
		{"snmpTraps child", model.OID{1, 3, 6, 1, 6, 3, 1, 1, 5, 1}, false},
		{"snmpTraps parent", model.OID{1, 3, 6, 1, 6, 3, 1, 1}, false},
		{"nil", nil, false},
		{"empty", model.OID{}, false},
	}
...
func TestResolveQualifiedNamedNumberComponent(t *testing.T) {
	...
	t.Run("falls back to createNamedChild when not found", func(t *testing.T) {
		defMod := &module.Module{Name: "TEST-MIB"}
		resolvedMod := model.NewModule("TEST-MIB")
		ctx := newResolverContext(nil, model.ResolverNormal, model.DefaultConfig())
		ctx.modules = []*module.Module{defMod}
...
func TestCreateNamedChild(t *testing.T) {
	t.Run("non-last sets name and module and kind", func(t *testing.T) {
		...
		testutil.Equal(t, model.KindNode, child.Kind(), "kind")
	...
	t.Run("last component does not set name or kind", func(t *testing.T) {
		...
		testutil.Equal(t, model.KindInternal, child.Kind(), "kind should remain Internal for last component")
	})
	...
	t.Run("does not override non-Internal kind", func(t *testing.T) {
		...
		existing := model.GetOrCreateChild(parent, 3)
		...
		child, ok := createNamedChild(ctx, def, parent, "org", 3, false)
		testutil.True(t, ok, "expected ok")
		testutil.Equal(t, model.KindScalar, child.Kind(), "kind should not be overridden when not Internal")
	})
	...
	t.Run("nil parent uses root", func(t *testing.T) {
		...
		testutil.Equal(t, model.GetOrCreateChild(ctx.mib.Root(), 1), child, "expected root child")
	})
	...
	t.Run("non-last does not overwrite preferred owner", func(t *testing.T) {
		...
		baseResolved := model.NewModule("SNMPv2-SMI")
		vendorResolved := model.NewModule("VENDOR-MIB")
		...
		existing := model.GetOrCreateChild(parent, 3)
		model.SetNodeName(existing, "org")
		model.SetNodeModule(existing, baseResolved)
...
func TestResolveOidComponentDispatch(t *testing.T) {
	...
	t.Run("QualifiedNamedNumber dispatches correctly", func(t *testing.T) {
		smiMod := &module.Module{Name: "SNMPv2-SMI"}
		defMod := &module.Module{Name: "TEST-MIB"}
		resolvedDefMod := model.NewModule("TEST-MIB")
		ctx := newResolverContext(nil, model.ResolverNormal, model.DefaultConfig())
		ctx.modules = []*module.Module{smiMod, defMod}
		ctx.moduleIndex["SNMPv2-SMI"] = []*module.Module{smiMod}
