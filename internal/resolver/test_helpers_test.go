...
func requireTypeDescriptionAndReference(t testing.TB, m *model.Mib, name string) *model.Type {
	t.Helper()
	typ := m.Type(name)
	if typ == nil {
...
func requireNodeDescription(t testing.TB, m *model.Mib, name string) *model.Node {
	t.Helper()
	node := m.Node(name)
	if node == nil {
...
func requireNodeReference(t testing.TB, m *model.Mib, name string) *model.Node {
	t.Helper()
	node := m.Node(name)
	if node == nil {
		t.Fatalf("node %q not found", name)
