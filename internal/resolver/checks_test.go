...
func TestCheckIndexElementNoSize(t *testing.T) {
	t.Run("OCTET STRING index without SIZE emits diagnostic", func(t *testing.T) {
		...
		idxObj := model.NewObject("strIndex")
		...
		model.SetNodeName(idxNode, "strIndex")
...
func TestIsLegalIndexBasetype(t *testing.T) {
	legal := []model.BaseType{
		model.BaseInteger32, model.BaseUnsigned32, model.BaseGauge32,
		model.BaseTimeTicks, model.BaseIpAddress, model.BaseOctetString, model.BaseOpaque,
		model.BaseBits, model.BaseObjectIdentifier,
	}
	for _, b := range legal {
		testutil.True(t, isLegalIndexBasetype(b), b.String()+" should be legal")
	}

	illegal := []model.BaseType{model.BaseCounter32, model.BaseCounter64, model.BaseSequence, model.BaseUnknown}
...
func TestCheckIndexIllegalBasetype(t *testing.T) {
	...
	idxObj := model.NewObject("badIndex")
	typ := model.NewType("Counter64")
	model.SetTypeBase(typ, model.BaseCounter64)
	model.SetObjectType(idxObj, typ)
	model.AddModuleObject(resolvedMod, idxObj)

	root := ctx.mib.Root()
	idxNode := buildOIDPath(root, 1, 1, 1, 1)
	model.SetNodeName(idxNode, "badIndex")
	model.SetNodeObject(idxNode, idxObj)
...
func TestCheckIndexOIDLength(t *testing.T) {
	t.Run("exceeds 128 sub-identifiers", func(t *testing.T) {
		...
		idxObj := model.NewObject("bigIndex")
	...
	t.Run("within 128 sub-identifiers", func(t *testing.T) {
		...
		idxObj := model.NewObject("intIndex")
		...
		model.SetNodeName(idxNode, "intIndex")
		model.SetNodeObject(idxNode, idxObj)
		...
		rowObj := model.NewObject("okEntry")
		model.AddModuleObject(resolvedMod, rowObj)
		model.SetObjectIndex(rowObj, []model.IndexEntry{{
			Object:   idxObj,
			Encoding: model.IndexEncodingInteger,
		}})

		rowNode := buildOIDPath(root, 1, 3, 6, 1, 2, 1, 100, 1, 1)
		model.SetNodeName(rowNode, "okEntry")
		model.SetNodeObject(rowNode, rowObj)
...
func TestIndexElementSubIds(t *testing.T) {
	tests := []struct {
		...
		{
			name:   "integer",
			entry:  model.IndexEntry{Object: makeTypedObject(model.BaseInteger32, nil), Encoding: model.IndexEncodingInteger},
			want:   1,
			wantOK: true,
		},
		{
			name:   "ip address",
			entry:  model.IndexEntry{Object: makeTypedObject(model.BaseIpAddress, nil), Encoding: model.IndexEncodingIpAddress},
			want:   4,
			wantOK: true,
		},
		{
		...
		},
		{
			...
			entry:  model.IndexEntry{Object: makeTypedObject(model.BaseObjectIdentifier, nil), Encoding: model.IndexEncodingLengthPrefixed},
			want:   129,
			wantOK: true,
		},
		{
			name:   "OID implied",
			entry:  model.IndexEntry{Object: makeTypedObject(model.BaseObjectIdentifier, nil), Encoding: model.IndexEncodingImplied},
			want:   128,
			wantOK: true,
		},
		{
			name:   "nil object",
			entry:  model.IndexEntry{Encoding: model.IndexEncodingInteger},
			want:   0,
			wantOK: false,
		},
		{
			name:   "unknown encoding",
			entry:  model.IndexEntry{Object: makeTypedObject(model.BaseOctetString, nil), Encoding: model.IndexEncodingUnknown},
			want:   0,
			wantOK: false,
		},
	}
...
func TestCheckIndexAccessAndDefval(t *testing.T) {
	tests := []struct {
		...
		{"SMIv2 read-only emits index-accessible", types.LanguageSMIv2, model.AccessReadOnly, false, false, types.DiagIndexAccessible},
		{"SMIv2 not-accessible is ok", types.LanguageSMIv2, model.AccessNotAccessible, false, false, ""},
		{"SMIv1 not-accessible emits index-not-accessible", types.LanguageSMIv1, model.AccessNotAccessible, false, false, types.DiagIndexNotAccessible},
		{"SMIv1 read-only is ok", types.LanguageSMIv1, model.AccessReadOnly, false, false, ""},
		{"INDEX with DEFVAL emits index-defval", types.LanguageSMIv2, model.AccessNotAccessible, true, true, types.DiagIndexDefval},
	}
