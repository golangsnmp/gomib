...
func checkTableRowNaming(ctx *resolverContext, objRefs []objectTypeRef) {
	for _, ref := range objRefs {
		...
		case model.KindRow:
			...
			if node.Parent() != nil && node.Parent().Kind() == model.KindTable {
		...
		for _, td := range mod.TypeDefs {
			...
			if td.DisplayHint != "" {
				...
				case model.BaseInteger32, model.BaseUnsigned32, model.BaseGauge32, model.BaseTimeTicks:
					valid = model.IsValidIntegerHint(td.DisplayHint)
				case model.BaseOctetString, model.BaseOpaque:
					valid = model.IsValidOctetStringHint(td.DisplayHint)
				default:
				...
				switch base {
				case model.BaseOctetString, model.BaseInteger32, model.BaseUnsigned32, model.BaseGauge32:
...
// intermediate nodes (model.KindInternal) that have children with actual
// definitions.
func checkNodeImplicit(ctx *resolverContext) {
	...
	for node := range root.Subtree() {
		if node.IsRoot() || node.Kind() != model.KindInternal {
			continue
		}
		if node.Name() != "" {
			continue
...
func checkModuleIdentityRegistration(ctx *resolverContext) {
	...
	mgmtPrefix := model.OID{1, 3, 6, 1, 2}
	// Controlled registration points.
	mib2Prefix := model.OID{1, 3, 6, 1, 2, 1}
	transmissionPrefix := model.OID{1, 3, 6, 1, 2, 1, 10}
	snmpModulesPrefix := model.OID{1, 3, 6, 1, 6, 3}
