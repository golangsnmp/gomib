...
func TestRegisterModules_LanguageSetOnResolved(t *testing.T) {
	...
	testutil.Equal(t, model.LanguageSMIv1, resolved.Language(), "resolved language")
}
...
func TestModuleImports_Grouping(t *testing.T) {
	...
	t.Run("single import", func(t *testing.T) {
		m := model.NewModule("TEST-MIB")
		model.SetModuleRawImports(m, []module.Import{
		...
		testutil.Equal(t, got[0].Symbols[0].Span, model.Span{Start: 10, End: 20}, "span preserved")
	})
...
func TestRegisterModules_DiagnosticsForwarded(t *testing.T) {
	...
	userMod := &module.Module{
		...
		Diagnostics: []model.Diagnostic{
			{Severity: model.SeverityWarning, Code: "test-warning", Message: "test", Module: "MY-MIB"},
		},
	}
	ctx := newResolverContext(nil, model.ResolverNormal, model.DefaultConfig())
