...
func recordRecursiveOid(ctx *resolverContext, def oidDefinition, scc []graph.Symbol) {
	...
	ctx.recordUnresolved(types.DiagOidRecursive, def.mod, span, msg,
		model.UnresolvedRef{Kind: model.UnresolvedOID, Module: def.mod.Name, Reason: reasonRecursiveOid})
}
...
const (
	...
	defNotification                            // NOTIFICATION-TYPE or TRAP-TYPE with model.OID
...
func resolveNameComponent(ctx *resolverContext, def oidDefinition, name string, compSpan types.Span) (*model.Node, bool) {
	...
	ctx.recordUnresolved(types.DiagOidOrphan, def.mod, compSpan,
		fmt.Sprintf("unresolved model.OID: %q references unknown parent %q", def.defName(), name),
		model.UnresolvedRef{Kind: model.UnresolvedOID, Symbol: name, Module: modName(def.mod), Reason: reasonUnknownParent})
	return nil, false
}

func resolveNamedNumberComponent(ctx *resolverContext, def oidDefinition, currentNode *model.Node, name string, number uint32, isLast bool) (*model.Node, bool) {
	if node, ok := ctx.lookupNode(def.mod, name); ok {
		return node, true
	}
...
func resolveQualifiedNameComponent(ctx *resolverContext, def oidDefinition, moduleName, name string, compSpan types.Span) (*model.Node, bool) {
	if node, ok := ctx.lookupNodeByModuleName(moduleName, name); ok {
		return node, true
	}
	ctx.recordUnresolved(types.DiagOidOrphan, def.mod, compSpan,
		fmt.Sprintf("unresolved model.OID: %q references unknown parent %q", def.defName(), moduleName+"."+name),
		model.UnresolvedRef{Kind: model.UnresolvedOID, Symbol: moduleName + "." + name, Module: modName(def.mod), Reason: reasonUnknownParent})
	return nil, false
}

func resolveQualifiedNamedNumberComponent(ctx *resolverContext, def oidDefinition, currentNode *model.Node, moduleName, name string, number uint32, isLast bool) (*model.Node, bool) {
	if node, ok := ctx.lookupNodeByModuleName(moduleName, name); ok {
...
func createNamedChild(ctx *resolverContext, def oidDefinition, currentNode *model.Node, name string, number uint32, isLast bool) (*model.Node, bool) {
	...
	if !isLast {
		...
		if shouldPreferModule(ctx, child.Module(), def.mod) {
			model.SetNodeName(child, name)
			model.SetNodeModule(child, ctx.moduleToResolved[def.mod])
			if child.Kind() == model.KindInternal {
				model.SetNodeKind(child, model.KindNode)
			}
		}
		model.RegisterMibNode(ctx.mib, name, child)
	}
	return child, true
}

func finalizeOidDefinition(ctx *resolverContext, def oidDefinition, node *model.Node, label string) {
	...
	// Prefer SMIv2 over SMIv1 when multiple modules define the same model.OID
	...
	if shouldPreferModule(ctx, currentMod, def.mod) {
		...
		case defNotification:
			model.SetNodeKind(node, model.KindNotification)
		case defObjectGroup, defNotificationGroup:
			model.SetNodeKind(node, model.KindGroup)
		case defModuleCompliance:
			model.SetNodeKind(node, model.KindCompliance)
		case defAgentCapabilities:
			model.SetNodeKind(node, model.KindCapability)
		}
		...
		model.SetNodeModule(node, newMod)
		if def.kind == defModuleIdentity {
			model.SetModuleOID(newMod, node.OID())
		}
	}
	...
	case defValueAssignment, defObjectIdentity, defModuleIdentity:
		model.AddModuleNode(newMod, node)
	default:
	...
	model.RegisterMibNode(ctx.mib, label, node)
...
func resolveNumericComponent(ctx *resolverContext, parent *model.Node, arc uint32) *model.Node {
	if parent != nil {
		return model.GetOrCreateChild(parent, arc)
	}
	return model.GetOrCreateChild(ctx.mib.Root(), arc)
}
	...
	for _, def := range defs {
		...
		if !found {
			ctx.recordUnresolved(types.DiagOidOrphan, def.mod, span,
				fmt.Sprintf("unresolved model.OID: %q references unknown parent %q", defName, enterprise),
				model.UnresolvedRef{Kind: model.UnresolvedOID, Symbol: enterprise, Module: modName(def.mod), Reason: reasonUnknownParent})
			continue
		}
		...
		if isSnmpTrapsOID(enterpriseNode.OID()) {
			...
			trapNode = model.GetOrCreateChild(zeroNode, trapNumber)
		}
		...
		if shouldPreferModule(ctx, trapNode.Module(), def.mod) {
			model.SetNodeName(trapNode, defName)
			model.SetNodeKind(trapNode, model.KindNotification)
			model.SetNodeModule(trapNode, ctx.moduleToResolved[def.mod])
		}
		ctx.registerModuleNodeSymbol(def.mod, defName, trapNode)
		model.RegisterMibNode(ctx.mib, defName, trapNode)
...
func lookupOrCreateWellKnownRoot(ctx *resolverContext, name string) (*model.Node, bool) {
...
var snmpTrapsOID = model.OID{1, 3, 6, 1, 6, 3, 1, 1, 5}

// isSnmpTrapsOID returns true if oid matches the snmpTraps model.OID,
// identifying the enterprise node for the six standard generic traps.
func isSnmpTrapsOID(oid model.OID) bool {
...
func isRegisteredKind(k model.Kind) bool {
	switch k {
	case model.KindScalar, model.KindTable, model.KindRow, model.KindColumn,
		model.KindNotification, model.KindGroup, model.KindCompliance, model.KindCapability:
		return true
	default:
		return false
	}
...
func shouldPreferModule(ctx *resolverContext, currentMod *model.Module, srcMod *module.Module) bool {
	if currentMod == nil {
		return true
	}
